        ...
        <div className="max-w-5xl mx-auto text-center relative z-10">
          ...
          <div className="mt-8 sm:mt-10">
            <DownloaderBox
              ...
              onResult={(res) => {
                if (res.type || res.mediaType) {
                  const raw = String(res.type || res.mediaType).toLowerCase();
                  const detected = (raw === 'carousel' || raw === 'photo') ? 'photo' : (raw === 'highlight' ? 'highlights' : (raw === 'reels' ? 'reels' : (raw === 'stories' ? 'stories' : (raw === 'igtv' ? 'igtv' : 'video'))));
                  setCurrentTab(detected as MediaType);
                }
